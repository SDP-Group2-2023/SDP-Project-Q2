...
std::string getFileName(const std::string &graph_path) {
    std::string result;
    int pos = graph_path.find_last_of('/') + 1;

    result.append(graph_path.cbegin() += pos, graph_path.cend());

    return result;
}

void calculatePartitionStats(const std::vector<int> &partition_size, unsigned long *min_partition, unsigned long *max_partition, unsigned long *avg_partition,
                             unsigned long *median_partition, unsigned long long *std_dev_partition) {
...
void read_test(std::string &graph_path, int num_iterations) {
    ...
    auto filename = std::string("read_test_").append(getFileName(graph_path));
...
void seq_test(std::string &graph_path, int num_iterations) {
    ...
    auto filename = std::string("seq_test_").append(getFileName(graph_path));
    filename      = filename.substr(0, filename.find_last_of('.')).append(".csv");
 ...
 */
void par_test(std::string &graph_path, int num_iterations, int num_threads) {
    std::cout << "PARALLEL PARTITIONING TEST" << std::endl;

    auto g = loadFromFile(graph_path);

    auto filename = std::string("par_test_").append(getFileName(graph_path));
    filename      = filename.substr(0, filename.find_last_of('.')).append(".csv");
...
int main(int argc, char **argv) {
    if (argc < 3) {
        std::cout << "Usage: " << argv[0] << " <input file> [reading|sequential|parallel [<num_threads>]]" << std::endl;
